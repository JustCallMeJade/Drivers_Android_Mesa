...
# If you don't have a needoh! You're a weirdo!, If you don't got a needoh! You're not gnarly!
...
# Bindgen uses LLVM 22/libclang, but must parse the headers
# as AArch64 Android rather than host x86_64.
export BINDGEN_CLANG_PATH=/usr/bin/clang-22
export BINDGEN_EXTRA_CLANG_ARGS="--target=aarch64-linux-android36 --sysroot=$sysroot"

# ============================================================
# MESON ANDROID CROSS FILE
# ============================================================
...
cpp_args = [
    '--sysroot=$sysroot',
    '-include',
    '$workdir/mesa/src/util/u_gralloc/force_aosp_abi.h',
...
c_link_args = [
    ...
    '-L$ndk_home/lib/clang/21/linux/aarch64',
    '-llog',
    '-lsync'
]
...
# IMPORTANT:
# LLVM 22 is used by bindgen, while the headers are parsed
# for the Android AArch64 target.
bindgen_clang_properties = '--target=aarch64-linux-android36 --sysroot=$sysroot'
...
ln -sf \
    "build-host/src/compiler/clc/mesa_clc" \
    "build-host/src/compiler/clc/mesa-clc"

ln -sf \
    "build-host/src/compiler/spirv/vtn_bindgen2" \
    "build-host/src/compiler/spirv/vtn-bindgen2"
